...
class PodService:
    ...
    def create_pod(self, db: Session, req: CreatePodRequest) -> ProofOfDelivery:
        ...
        latest = self.tracking.get_latest_status(req.tracking_number)

            # Si deja DELIVERED => etat final, on NE push rien (sinon 409)
        if latest != "DELIVERED":
                # Option simple: si pas encore DELIVERED, on peut pousser DELIVERED
                self.tracking.add_event(
                    TrackingEventCreate(
                        tracking_number=req.tracking_number,
                        status="DELIVERED",
                        source="SYSTEM",  # ou "COURIER" si tu veux
                        message=f"POD created/confirmed by {req.receiver_name}",
                    )
                )

        return pod
    def list_pods(self, db: Session):
